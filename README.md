...
`MoonClient` owns the runtime thread and `connect` returns immediately. Init is one-time per session; readiness arrives as `LifecycleEvent::Ready`. After Init, reconnect restore, market refresh, saved subscriptions, orderbook full resync, trades gap recovery, and pending Engine API dispatch are owned by the library until `disconnect()` or drop. See [`docs/active_lib.md`](docs/active_lib.md) for the maintained-state contract.
...
- **Full profile** (destructive/stress, `allow_mutation = true`) — two live clients, `err_emu` packet loss (10% → 50%), chunked candles under loss, settings/strategy broadcast, an emulator + real SOLUSDT cancel gate, forced reconnect and post-reconnect delivery, plus detailed sliced/retry/parse/PMTU/CPU diagnostics.

Set `MOONPROTO_FIRETEST_ERR_EMU=0` to disable the client-side packet-loss emulation (default 10%). FireTest writes strategy diagnostics under `target/` (`firetest_strategy_info_<profile>.txt`, `firetest_strategy_raw/`). See `tests/README.md` for the full test-layer map.
