            ...
            match cmd {
                ...
                Command::TradesStream => {
                    if let Some(pkt) = commands::trades_stream::parse_trades_packet(data) {
                        ...
                        trades_count += 1;
                        if trades_count <= 3 || trades_count.is_multiple_of(10) {
                            println!(
                                "[trades] pkt#{} sections={} trades={}",
                                pkt.packet_num,
                                pkt.sections.len(),
                                total_trades
                            );
                ...
                Command::OrderBook => {
                    if let Some(book) = commands::order_book::parse_order_book_packet(data) {
                        book_count += 1;
                        if book_count <= 3 || book_count.is_multiple_of(10) {
                            println!(
                                "[book] mkt={} seq={} full={} buys={} sells={}",
                                book.market_index,
                                book.seq,
                                book.is_full,
                                book.buys.len(),
                ...
                Command::Balance => {
                    ...
                    if data.len() > 11 {
                        ...
                        if let Some(bal) = commands::balance::parse_balance(cmd_id, &data[11..]) {
                            println!(
                                "[balance] cmd={} epoch={} items={} btc={:.4}",
                                cmd_id,
                                bal.epoch,
                                bal.items.len(),
                                bal.btc_balance_total
                            );
                        }
                    }
                }
                ...
                Command::API => {
                    if data.len() > 11 {
                        println!(
                            "[api] Response ({} bytes) first_bytes={:02x?}",
                            data.len(),
