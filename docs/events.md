...
| `Balance` | Applies full/incremental balance updates to live `Market` objects and to the secondary `BalancesState` table. Arbitrage relay payloads update `Market::arb_slots`; `Event::Arb` is also emitted as a diagnostic/protocol event after filtering through the current server market-index map. Internal/base/request balance packets are consumed without user-facing events. |
